...
def get_user_orders(jwt: AuthJWT = Depends(dependecies.check_token), db: Session = Depends(get_db)):
...
def get_order(id: int, jwt: AuthJWT = Depends(dependecies.check_token), db: Session = Depends(get_db), order: None = Depends(dependecies.check_order_owner)):
